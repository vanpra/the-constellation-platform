...
export function SaltSection(props: SaltSectionProps) {
    ...
    return (
        post?.salt_stage ?
            ...
            >
                <div> {post?.prev_salt_post && <RedRoundedButton className="text-sm" onClick={prevPostOnClick} text={"<-- " + post?.prev_salt_post?.title!} />} </div>
                {post?.salt_stage && <div className=" text-center"> Salt stage:<p className="font-bold">{post?.salt_stage}</p> </div>}
                <div> {post?.next_salt_post && <RedRoundedButton className="text-sm float-right " onClick={nextPostOnClick} text={post?.next_salt_post?.title! + " -->"} />} </div>
            </div>
            : <></>
    );
}
